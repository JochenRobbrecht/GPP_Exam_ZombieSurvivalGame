...
namespace Elite
{
	...
	public:
		...
		template<typename T> bool GetData(const std::string& name, T& data,bool printWarning = true)
		{
			if (m_BlackboardData.find(name) == m_BlackboardData.end())
			{
				if (printWarning)printf("WARNING: Data '%s' of type '%s' not found in Blackboard \n", name.c_str(), typeid(T).name());
				return false;
			}
			BlackboardField<T>* p = dynamic_cast<BlackboardField<T>*>(m_BlackboardData[name]);
			if (p != nullptr)
